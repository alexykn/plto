...
#[derive(Parser, Debug)]
#[command(name = "plto")]
#[command(about = "Scaffolds projects from ~/.config/plto", long_about = None)]
...
pub(crate) enum Commands {
    ...
    Val(ValArgs),
    /// Open a configured template override or local plto.toml in editor
    Config {
...
mod tests {
    ...
    fn parses_group_and_set_shorthands_for_validation() {
        let cli = Cli::parse_from(["plto", "val", "py-api", "-g", "docker", "-s", "docker=true"]);
