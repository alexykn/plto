use plto::plugins::install::PluginInstallBackend;
use plto::{
...
pub(crate) fn run_command(command: Commands) -> anyhow::Result<()> {
    match command {
        ...
        Commands::Config { template_name } => plto::edit_config(&template_name),
        Commands::List { verbose } => plto::display_templates(verbose),
...
fn run_init(args: InitArgs) -> anyhow::Result<()> {
    ...
    let summary = plto::run(RunOptions {
        template,
...
fn run_validate(args: ValArgs) -> anyhow::Result<()> {
    ...
    plto::validate(ValidateOptions { template })
}

fn run_plugin(command: PluginCommands) -> anyhow::Result<()> {
    match command {
        PluginCommands::List => plto::display_plugins(),
        PluginCommands::Install(args) => run_plugin_install(args),
        PluginCommands::Register { name, command } => {
            plto::register_plugin(PluginRegisterOptions { name, command })
        }
        PluginCommands::Remove { name } => plto::remove_plugin(&name),
    }
}
...
fn run_plugin_install(args: PluginInstallArgs) -> anyhow::Result<()> {
    ...
    plto::install_plugin(PluginInstallOptions { name, backend })
}
