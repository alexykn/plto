...
pub(crate) struct GlobalConfig {
    #[serde(default)]
    pub(crate) plto: GlobalPltoConfig,
    #[serde(default)]
...
#[serde(deny_unknown_fields)]
pub(crate) struct GlobalPltoConfig {
    #[serde(default)]
...
impl GlobalConfig {
    pub(crate) fn validate(&self) -> Result<()> {
        ...
        validate_git_hosts(&self.plto.git_hosts)
    }
}
...
/// Returns the directory that stores Plto's configuration files.
///
/// # Errors
/// Returns an error if the user's home directory cannot be determined.
pub(crate) fn get_global_plto_dir() -> Result<PathBuf> {
    let base_dirs = BaseDirs::new().context("Could not find home directory")?;
    Ok(base_dirs.home_dir().join(".config/plto"))
}

/// Returns the global Plto config path.
///
/// # Errors
/// Returns an error if the user's home directory cannot be determined.
pub(crate) fn get_global_config_path() -> Result<PathBuf> {
    Ok(get_global_plto_dir()?.join("config.toml"))
}
