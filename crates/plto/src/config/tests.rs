...
[plugin_registry.uv]
command = "~/.local/share/plto/plugins/bin/plto-plugin-uv"
...
"#;
    ...
    assert_eq!(
        ...
        PathBuf::from("~/.config/plto/template_configs/api.toml")
    );
    assert_eq!(
        config.plugin_registry["uv"].command,
        PathBuf::from("~/.local/share/plto/plugins/bin/plto-plugin-uv")
    );
}
