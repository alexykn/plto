...
impl WorkspaceBuilder {
    pub(crate) fn from_source(source_path: &Path) -> Result<Self> {
        ...
        for entry in WalkDir::new(source_path) {
            ...
            if is_reserved_plto_config_path(&rel_path) {
                continue;
            }
...
fn is_reserved_plto_config_path(rel_path: &Path) -> bool {
    if rel_path
        .parent()
...
mod tests {
    ...
    fn temp_root(label: &str) -> PathBuf {
        let unique = SystemTime::now()
            ...
            .unwrap()
            .as_nanos();
        let root = std::env::temp_dir().join(format!("plto-builder-{label}-{unique}"));
        create_dir_all(&root).unwrap();
        root
    }

    #[test]
    fn identifies_root_plto_config_files_as_reserved() {
        assert!(is_reserved_plto_config_path(Path::new("plto.toml")));
        assert!(is_reserved_plto_config_path(Path::new("plto.docker.toml")));
    }

    #[test]
    fn does_not_reserve_nested_or_non_config_files() {
        assert!(!is_reserved_plto_config_path(Path::new(
            "groups/plto.docker.toml"
        )));
        assert!(!is_reserved_plto_config_path(Path::new("plto.template")));
    }
